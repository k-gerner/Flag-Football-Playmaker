...
export function PlayLibrary({
  ...
  const [canScrollPlaysLeft, setCanScrollPlaysLeft] = useState(false);
  const [canScrollPlaysRight, setCanScrollPlaysRight] = useState(false);
  const playSetRailRef = useRef<HTMLDivElement | null>(null);
  const playRailRef = useRef<HTMLDivElement | null>(null);
  ...
  const activePlay = plays.find((play) => play.id === activePlayId) ?? null;
  const playPickerLabel = activePlay?.name ?? (activePlaySet ? "Select a Play" : "Choose a Play Set first");
  ...
  function updatePlayRailScrollState() {
    const rail = playRailRef.current;
    if (!rail || !isPlayPickerOpen) {
      setCanScrollPlaysLeft(false);
      setCanScrollPlaysRight(false);
      return;
    }

    const maxScrollLeft = Math.max(rail.scrollWidth - rail.clientWidth, 0);
    setCanScrollPlaysLeft(rail.scrollLeft > 4);
    setCanScrollPlaysRight(maxScrollLeft - rail.scrollLeft > 4);
  }

  function scrollPlayRail(direction: "left" | "right") {
    const rail = playRailRef.current;
    if (!rail) {
      return;
    }

    const firstRailItem = rail.querySelector<HTMLElement>("[data-play-rail-item='true']");
    const step = firstRailItem ? firstRailItem.getBoundingClientRect().width + 8 : 240;
    rail.scrollBy({
      left: direction === "left" ? -step : step,
      behavior: "smooth",
  ...
  function handlePlaySelect(playId: string) {
    onSelectPlay(playId);
    setIsPlayPickerOpen(false);
  }

  function handleCreatePlayClick() {
    onCreatePlay();
    setIsPlayPickerOpen(false);
  }
  ...
  useEffect(() => {
    ...
    const frameId = window.requestAnimationFrame(updatePlayRailScrollState);
    window.addEventListener("resize", updatePlayRailScrollState);

    return () => {
      window.cancelAnimationFrame(frameId);
      window.removeEventListener("resize", updatePlayRailScrollState);
    };
  }, [isPlayPickerOpen, plays.length]);
    ...
    <aside className="glass-panel flex h-full min-w-0 flex-col overflow-hidden rounded-[28px] border border-white/70 p-4 shadow-panel">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex flex-wrap justify-end gap-1.5">
                          ...
                          <span className="rounded-full bg-ink-950 px-3 py-1 text-xs font-semibold text-white">
                            {playSet.settings.layout.rowsPerPage}x{playSet.settings.layout.columnsPerPage}
                          </span>
                        </div>
                      </div>
      ...
      <div className="mt-5 border-t border-black/8 pt-4">
        ...
        {activePlaySet ? (
          <>
            <button
              aria-controls="play-picker-rail"
              aria-expanded={isPlayPickerOpen}
              className="flex w-full items-center justify-between rounded-[22px] border border-black/8 bg-white/75 px-4 py-3 text-left transition hover:border-ember-500/35 hover:bg-white"
              data-testid="play-picker-trigger"
              onClick={() => setIsPlayPickerOpen((current) => !current)}
              type="button"
            >
              <span className="font-display text-base font-bold text-ink-950">{playPickerLabel}</span>
              <span className="flex items-center gap-2 text-sm font-semibold text-ink-950/65">
                {plays.length} play{plays.length === 1 ? "" : "s"}
                <ChevronIcon open={isPlayPickerOpen} />
              </span>
            </button>
              ...
              <div className="relative mt-3">
                {canScrollPlaysLeft ? (
                  <>
                    <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-16 bg-gradient-to-r from-[rgba(247,242,232,0.96)] to-transparent" />
                    <button
                      aria-label="Scroll plays left"
                      className="absolute left-2 top-1/2 z-20 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-black/10 bg-white/90 text-ink-950 shadow-sm transition hover:border-ember-500/35 hover:bg-white"
                      onClick={() => scrollPlayRail("left")}
                      type="button"
                    >
                      <ScrollArrowIcon direction="left" />
                    </button>
                ...
                <div
                  className="w-full max-w-full overflow-x-auto overflow-y-hidden pb-1"
                  data-testid="play-picker-rail"
                  id="play-picker-rail"
                  onScroll={updatePlayRailScrollState}
                  ref={playRailRef}
                >
                  <div className="inline-flex min-w-max items-start gap-2 pr-0.5">
                    {plays.length === 0 ? (
                    ...
                    {plays.map((play, index) => {
                      ...
                      return (
                        <article
                          ...
                          key={play.id}
                        >
                          <button className="block w-full flex-1 text-left" onClick={() => handlePlaySelect(play.id)} type="button">
                ...
                {canScrollPlaysRight ? (
                  <>
                    <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-16 bg-gradient-to-l from-[rgba(247,242,232,0.96)] to-transparent" />
                    <button
                      aria-label="Scroll plays right"
                      className="absolute right-2 top-1/2 z-20 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full border border-black/10 bg-white/90 text-ink-950 shadow-sm transition hover:border-ember-500/35 hover:bg-white"
                      onClick={() => scrollPlayRail("right")}
                      type="button"
                    >
                      <ScrollArrowIcon direction="right" />
                    </button>
